...
final class ConnectErrorTests: XCTestCase {
    ...
    func testConnectCompressedEndStreamWithError() throws {
        let json: [UInt8] = Array(#"{"error":{"code":"permission_denied","message":"Reached message rate limit. Resets in: 5m44s"}}"#.utf8)
        let compressed = try gzip(json)
        var body: [UInt8] = [0x03]
        var len = UInt32(compressed.count).bigEndian
        withUnsafeBytes(of: &len) { body.append(contentsOf: $0) }
        body.append(contentsOf: compressed)

        let err = ConnectError.scanConnectFrames(body)
        XCTAssertEqual(err?.code, "permission_denied")
        XCTAssertEqual(err?.resetIn, 5 * 60 + 44)
    }

    func testConnectCompressedEmptyEndStreamNoError() throws {
        let compressed = try gzip(Array(#"{}"#.utf8))
        var body: [UInt8] = [0x03]
        var len = UInt32(compressed.count).bigEndian
        withUnsafeBytes(of: &len) { body.append(contentsOf: $0) }
        body.append(contentsOf: compressed)
    ...
    private func gzip(_ bytes: [UInt8]) throws -> [UInt8] {
        ...
        bytes.withUnsafeBufferPointer { input in
            stream.next_in = UnsafeMutablePointer<Bytef>(mutating: input.baseAddress)
            stream.avail_in = uInt(bytes.count)

            repeat {
                let start = output.count
                output.append(contentsOf: repeatElement(0, count: chunkSize))
                ...
                if produced < chunkSize {
                    output.removeLast(chunkSize - produced)
                }
            } while status == Z_OK
        }

        XCTAssertEqual(status, Z_STREAM_END)
        guard status == Z_STREAM_END else { throw NSError(domain: "gzip", code: Int(status)) }
        return output
    }
}
