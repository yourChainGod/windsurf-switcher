...
//  App
//
//  账号管理页面：app 过滤 + 状态统计 + API 入口提示 + 卡片列表。
//
...
private struct FilterBar: View {
    ...
    private func icon(for f: AppFilter) -> String {
        switch f {
        case .all: return "circle.grid.2x2"
        case .stable: return "wind"
        case .next: return "wind.snow"
        }
    }
}
...
private struct AppPill: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 7)
                    .stroke(selected ? Color.cyan.opacity(0.45) : Color.clear, lineWidth: 1)
            )
    ...
    private var bg: Color {
        if selected { return Color.cyan.opacity(0.18) }
...
private struct SummaryStrip: View {
    ...
    var body: some View {
        let s = state.summary
        let total = state.filteredAccounts.count
        HStack(spacing: 6) {
