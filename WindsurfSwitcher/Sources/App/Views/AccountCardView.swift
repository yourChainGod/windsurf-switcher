...
//    - 配额：daily% / weekly% 横条
//    - 操作：切号（按钮，含 segment=All 时弹 menu 选 app）/ 刷新 / 删除（带确认）/ 重命名
//
...
struct AccountCardView: View {
    ...
    private func handleTap() {
        ...
        // 切号：按 segment 选 app；segment=All 时默认 stable，魔尊在卡片右上角操作菜单可显式选另一个
        let app = state.filter.targetApp ?? account.lastUsedApp ?? .stable
